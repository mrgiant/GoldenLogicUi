...
(function (window, document) {
    ...
    Object.assign(Plugin.prototype, {
        init: function () {
            ...
            if (
                this.transInputs.length === 0 ||
                this.langSelectors.length === 0
            ) {
                return false;
            }
        ...
        setup: function () {
            ...
            this.langSelectors.forEach(function (btn) {
                btn.addEventListener(
                    "change",
                    _this.selectLanguage.bind(_this)
                );
            });
        },
                ...
                if (inpUsr) {
                    ...
                    if (_this.settings.editing) {
                        if (
                            ...
                            inpUsr.classList.contains("tiny")
                        ) {
                            var $_mce = tinymce.get(
                                inpUsr.name
                            );

                            $_mce.on('change', function(e) {
                        ...
                        else if (inpUsr.classList.contains("CodeEditor")) {
                              

                            let cmEditorElement = inpUsr;
                           ...
                           editorView.dispatch({
                            effects: editorView.state.effects.of((tr) => {
                                if (tr.docChanged) {
                                    console.log("Document changed:", editorView.state.doc.toString());
                                    _this.prepareData(); // Call your function here
                                }
                            })
                        });
                            ...
                            inpUsr.addEventListener("change", function (e) {
                                _this.prepareData();
                            });
                        }
                    }
        ...
        returnLocale: function () {
            var selectedRadioButton = this.element.querySelector(
                'input[name="' +
                    this.settings.trans_selector_name +
                    '"]:checked'
            );
        ...
        loadLang: function (inp, lang) {
            ...
            if (!this.settings.editing) {
               ...
               // document.getElementById(inpUsr).value = _val;
                ...
                if (
                    document
                        .getElementById(inpUsr)
                        .classList.contains("tiny") &&
                    _mce &&
                    _mce.initialized
                ) {
                ...
                else if (document.getElementById(inpUsr).classList.contains("CodeEditor")) {
                    ...
                    editorView.dispatch({ changes: { from: 0, to: editorView.state.doc.length, insert: _val } })
