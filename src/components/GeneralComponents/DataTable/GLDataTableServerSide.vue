<template>
  ...
  <div class="p-0">
    ...
    <div class="flex flex-wrap mb-3 gap-1 lg:gap-0!">
      ...
      <gl-button @click="exportToExcel" tag="button" button_type="light" :has_border_reduced="false" classes="rounded-lg lg:rounded-none"
...
<div :id="'db_' + Random_string"  v-show="isOpenColumnVisibility" class="z-10 absolute  w-42 bg-white rounded-lg shadow-sm dark:bg-gray-700">
    <ul class="p-3 space-y-1 text-sm text-gray-700 dark:text-gray-200" :aria-labelledby="'db_' + Random_string">
      ...
      <gl-button v-if="enable_select_deselect_delete && enable_delete && can(`delete_${xprops?.permission}`)" tag="button" @click="deleteSelected" button_type="red"
